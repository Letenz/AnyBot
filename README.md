...
目前支持 [OpenAI Codex CLI](https://github.com/openai/codex)、[Cursor CLI](https://docs.cursor.com/cli) 和 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 作为 Provider。
...
- **多 Provider 架构** — 可插拔的 AI CLI 后端，当前支持 Codex CLI、Claude Code、Cursor CLI，未来可扩展更多
...
通过环境变量 `PROVIDER=codex`、`PROVIDER=cursor-cli` 或 `PROVIDER=claude-code` 切换默认 Provider，也可在 Web UI 中随时切换。
...
| `/provider <名称>` | 切换供应商，例如 `/provider cursor-cli` |
...
- 很适合在本机开代理后，让 Codex / Cursor / Telegram 统一走代理
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `PROVIDER` | `codex` | 使用的 Provider：`codex`、`cursor-cli`、`claude-code` |
...
可参考 `src/providers/codex.ts` 和 `src/providers/cursor-cli.ts` 作为实现模板。
