...
- **One-Click Setup** — Interactive `npm run setup` guides you through all configuration, auto-detects dependencies and selects Provider
...
`npm run setup` will guide you through:
- Detecting OS and base dependencies (Node.js, npm)
- **Choosing a default Provider** (Codex CLI / Gemini CLI / Cursor CLI / Qoder CLI / Claude Code)
- Checking the corresponding CLI or SDK configuration, with installation guidance
- Setting the working directory
- Configuring safety mode (Codex/Claude: Sandbox mode / Gemini: Approval Mode)
...
Configured in the `.env` file (generated by `npm run setup` or manually copied from `.env.example`).
...
| Variable | Default | Description |
|----------|---------|-------------|
| `CLAUDE_CODE_BIN` | `claude` | Local `claude` executable path; defaults to the logged-in Claude Code CLI |
...
Note: the SDK executes the file pointed to by `CLAUDE_CODE_BIN`; it does not read shell functions or aliases. If your `claude` command is a shell function wrapper, save equivalent logic as a script and point `CLAUDE_CODE_BIN` to that script.
...
├── setup.sh                # legacy macOS/Linux setup wizard (optional)
